...
import { Phone, X, Check } from "lucide-react-native";
...
export function RequestCard({
  ...
  return (
    <View className={`mb-4 rounded-2xl border border-textGray200 bg-white p-4 ${highlight ? "shadow" : ""}`}>
      <View className="flex-row items-center justify-between">
        <View>
          <Text className="text-base font-semibold text-textPrimary900">{buyerName}</Text>
          {buyerNote ? <Text className="text-sm text-textGray700">"{buyerNote}"</Text> : null}
        </View>
      ...
      <View className="mt-3 flex-row gap-3 rounded-2xl bg-textGray50 p-3">
        ...
        <View className="flex-1 gap-1">
          <Text className="text-base font-semibold text-textPrimary900">{bookTitle}</Text>
          <Text className="text-sm text-textGray700">Ngoại ngữ</Text>
          <View className="flex-row items-center justify-between">
            <View className="flex-row items-center gap-2">
              <View className="rounded-full bg-textPrimary50 px-3 py-1">
                <Text className="text-xs font-semibold text-textPrimary500">Chưa đọc</Text>
              </View>
      ...
      {showActions ? (
        <View className="mt-3 flex-row gap-3">
          <Pressable className="flex-1 items-center rounded-full border border-textGray300 bg-white py-3" onPress={onReject}>
            <Text className="text-sm font-semibold text-textGray700">Từ chối</Text>
          </Pressable>
          <Pressable className="flex-1 items-center rounded-full bg-textPrimary500 py-3" onPress={onApprove}>
        ...
        <View className="mt-3 flex-row items-center justify-between rounded-full bg-textGray100 px-4 py-3">
          <View className="flex-row items-center gap-2">
            ...
            <Text className="text-sm font-semibold text-textPrimary900">{footerLabel}</Text>
          </View>
      ...
      {!showActions && !footerLabel ? (
        <View className="mt-3 flex-row items-center gap-2 rounded-full bg-textGray100 px-4 py-3">
          ...
          <Text className="text-sm font-semibold text-textPrimary900">Bạn đã từ chối yêu cầu mua sách này.</Text>
        </View>
      ) : null}
    </View>
  );
